...
#Region "Filtre"
    ...
    Private Sub FiltrerLesProduits()
        ...
        'Exit Sub
        Me.VueSourceProduits.Filter = Function(p As Produit) MatcheCritères(p)
...
#Region "MatcheCritères"
    ...
    Private Function MatcheCritères(P As Produit) As Boolean

        'Return True

        Dim MatchMotsClés, MatchCodeLydic, MatchCodeFournisseur As Boolean

        MatchMotsClés = MatcheCritèresMotsClés(P, Me.CritèreMotsClés)
        If MatchMotsClés Then
            MatchCodeLydic = MatcheCritèresCodeLydic(P, Me.CritèreCodeLydic)
            If MatchMotsClés Then
                MatchCodeFournisseur = MatcheCritèresNumFournisseur(P, Me.CritèreRefFournisseur)
            End If
        End If
    ...
    Private Function MatcheCritèresCodeLydic(P As Produit, C As String) As Boolean
        Dim r = String.IsNullOrEmpty(C)
        r = r OrElse P.CodeLydic?.StartsWith(C, StringComparison.CurrentCultureIgnoreCase)
        Return r
    End Function

    Private Function MatcheCritèresNumFournisseur(P As Produit, C As String) As Boolean
        Dim r = String.IsNullOrEmpty(C)
        r = r OrElse P.RéférenceFournisseur.Contains(C)
        Return r
    End Function

    Private Function MatcheCritèresMotsClés(P As Produit, C As String) As Boolean
        Dim r = String.IsNullOrEmpty(C)
        If Not r Then
